...
timestamp = int(time.time())  # Use timestamp to make articles unique
test_articles = []
for i in range(1, 4):
    ...
    test_article = {
        "id": article_id,
        "title": f"API Test Article {timestamp}-{i}",
        "description": f"This is test article {timestamp}-{i} created via the API",
        "url": f"https://example.com/api-test-{timestamp}-{i}",
        "source_name": "API Test Source",
        "published_at": published_at,
        "content": f"Test content for API article {timestamp}-{i}",
        "author": "API Test Script",
        "raw_data": {"test": True, "source": "api", "timestamp": timestamp}
    }
...
try:
    ...
    if get_response.status_code == 200:
        ...
        # Print the first few
        for i, article in enumerate(saved_articles[:5]):
